...
def main():
    ...
    with PdfPages(PDF) as pdf:
        ...
        # Page 16 - AL-RNN paper summary
        text_page(pdf, "Comparison to the AL-RNN paper (Brenner et al., NeurIPS 2024)",
                  [
                      "Reference: Brenner, Hemmer, Monfared, Durstewitz,",
                      "'Almost-Linear RNNs Yield Highly Interpretable Symbolic Codes",
                      " in Dynamical Systems Reconstruction', NeurIPS 2024.",
                      "arxiv 2410.14240   (github.com/DurstewitzLab/ALRNN-DSR)",
                      "",
                      "### Their core claims",
                      "",
                      "1. The AL-RNN splits the hidden state into M-P linear and P ReLU",
                      "   units.  Training finds the MINIMAL P that still reconstructs",
                      "   the dynamics (P = 2-3 for Lorenz63 and Roessler).",
                      "",
                      "2. With P ReLU units, the state space is partitioned into at",
                      "   most 2^P linear regions by P axis-aligned hyperplanes",
                      "   { z : z_{M-P+j} = 0 }.  Each region is indexed by a P-bit",
                      "   symbol giving the sign pattern of the ReLU inputs.",
                      "",
                      "3. The symbolic sequence along a trajectory is a Markovian code",
                      "   that preserves topological invariants of the chaotic attractor",
                      "   (kneading-like data).  Within each region dynamics are purely",
                      "   linear, so fixed points, eigenvalues and stable/unstable",
                      "   manifolds can be enumerated analytically per region.",
                      "",
                      "4. In their experiments they use latent_dim=M=20 (their README",
                      "   default) with P=2 or 3.  Reconstruction metrics are MSE,",
                      "   state-space distance D_stsp, and power-spectrum error PSE.",
                  ])

        # Page 17 - Our three claims vs paper
        text_page(pdf, "Our three claims about the orthogonal AL-RNN",
                  [
                      "### (1) Better performance at the same (M, P)",
                      "",
                      "Across all 27 (M, P, alpha) cells we tested, the ortho variant",
                      "lowers final MSE at 26/27 cells (the sole exception is",
                      "M=10 P=2 alpha=1.0 where ortho is worse by 0.7 %).",
                      "",
                      "   M=4:   -50 to -73 % MSE at every P and alpha",
                      "   M=5:   -18 to -74 %",
                      "   M=10:  -10 to -31 % (alpha=0.5); 0 to -25 % (alpha=1.0)",
                      "   M=20:   -6 to -13 % (alpha=1.0 shared-B)",
                      "",
                      "### (2) Smaller latent space for the same performance",
                      "",
                      "Matching MSE within 15 % across M and P (shared frozen B):",
                      "",
                      "   ortho M=5  P=3  MSE 0.023   <->   vanilla M=20 P=3  MSE 0.022",
                      "      50 trainable params vs 440   (9x reduction)",
                      "",
                      "   ortho M=10 P=4  MSE 0.012   <->   vanilla M=20 P=4  MSE 0.012",
                      "      175 params vs 440            (2.5x reduction)",
                      "",
                      "   ortho M=5  P=4  MSE 0.016   <->   vanilla M=10 P=4  MSE 0.019",
                      "      50 params vs 120              (2.4x reduction)",
                      "",
                      "The Pareto plot shows the ortho cloud strictly below vanilla at",
                      "every parameter band, for both alpha=1.0 and alpha=0.5.",
                      "",
                      "### (3) The interpretability machinery transfers, with one caveat",
                      "",
                      "The ortho recurrence is still EXACTLY piecewise-linear.  Within a",
                      "fixed sign pattern of the P ReLU inputs, (Q z + b_Q)_j has fixed",
                      "sign for j in {M-P,..,M-1}, so phi_P is an affine function of z",
                      "and the whole step is affine.  Hence:",
                      "",
                      "   * Exactly 2^P linear regions, same alphabet size as vanilla.",
                      "   * Region boundaries are the P hyperplanes",
                      "        { z : q_j^T z + b_{Q,j} = 0 },  j = M-P,..,M-1",
                      "     where q_j is the j-th row of Q.  In vanilla these are axes",
                      "     of z; in ortho they are learned directions - an arbitrary",
                      "     orthogonal frame of P hyperplanes in R^M.",
                      "   * Region-wise eigenvalues, fixed points, stable/unstable",
                      "     manifolds, symbolic-dynamics analysis - all go through",
                      "     unchanged relative to these hyperplanes.",
                      "",
                      "Empirically (see next page) both models visit a comparable",
                      "number of symbols and produce similarly sparse symbolic codes.",
                      "At M=10 P=4 alpha=1.0 vanilla visits 9 of 16 regions (H=2.58",
                      "bits), ortho visits 10 of 16 (H=2.65 bits) - essentially tied.",
                      "",
                      "### The caveat",
                      "",
                      "For vanilla the j-th symbol bit corresponds to a single hidden",
                      "unit: 'unit M-P+j fires or not'.  For ortho the same bit",
                      "corresponds to a LEARNED linear combination of units:",
                      "'q_j^T z + b_{Q,j} > 0'.  The anatomical mapping 'one symbol",
                      "bit <-> one hidden unit' is lost.  The topological/dynamical",
                      "content of the symbolic code (which region, which transition)",
                      "is identical.",
                  ])

        # Page 18 - symbolic-code figure
        image_page(pdf, "symbolic_code_coloured_attractor.png",
                   "Symbolic code on the attractor - vanilla vs ortho (M=5, P=3)")
